    ...
    <div class="content-list">
        ...
        //doanh thu
            ...
            if($toDate != ''){
                ...
                $sql="SELECT total_price, status FROM order_header where shop_id= $shopId and create_date>='$fromDate' and create_date<'$toDate' and status != 'cancle'";
            } else {
                $sql="SELECT total_price, status FROM order_header where shop_id= $shopId and create_date>='$fromDate' and create_date<NOW() and status != 'cancle'";
            }
            $result=mysqli_query($con,$sql);
            while($tv_3=mysqli_fetch_array($result)){
                $total += $tv_3['total_price'];
                if($tv_3['status'] == 'resolve')
                    $danhan += $tv_3['total_price'];
            }
